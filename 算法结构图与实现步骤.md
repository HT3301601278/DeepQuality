...
flowchart LR
    ...
    B --> C["chronological_split或random_window流程"]
    C --> D["clean_missing_values"]
    D --> E["Standardizer.fit(训练部分)"]
    E --> F["clip到训练集3sigma范围"]
    F --> G["标准化X和Y"]
    G --> H["未加权训练窗口"]
    H --> I["nested_label_masks"]
    I --> J["compute_correlation_weights"]
    G --> K["make_windows / make_multiscale_windows"]
    J --> K
    K --> L["train/val/test窗口数据"]
```

实现步骤：

1. 从 `data.path` 读取 CSV，`target_column` 作为质量变量，其他列作为过程变量。
2. `chronological` 流程按时间顺序划分训练集、验证集、测试集，不打乱样本。
3. `random_window` 流程先清洗全量序列，构造原始窗口后再随机划分窗口样本。
4. 清洗缺失值时删除连续缺失长度大于 `5` 的行段；任一列缺失率大于等于 `0.01` 时报错；剩余缺失按列线性插值。
5. 只用训练部分计算输入和输出的均值、标准差、3sigma上下界。
6. 对训练集、验证集、测试集执行 3sigma 裁剪和标准化。
7. 先构造未加权训练窗口，再根据 `label_ratio` 和随机种子生成训练窗口的有标签掩码。
8. 在有标签训练窗口的 `current_u` 和目标值上计算过程变量与质量变量的 Pearson 和 Spearman 综合相关性权重。
9. 构造最终窗口时先对过程变量乘相关性权重，再展开成动态窗口。
10. 单尺度窗口使用 `window_size` 和 `quality_delay`，输入为 `x(t)`，标签为 `y(t+d)`。
11. 多尺度窗口使用多个 `(window_size, stride)`，不同尺度共用同一组窗口结束时刻。
...
5. `ar` 默认使用验证集残差拟合一阶自回归残差模型，也可通过 CLI 参数扩展阶数和岭回归系数。
6. `ema+ar` 先平滑预测，再对平滑残差拟合 AR 残差模型。
...
2. 按split_method执行时间顺序划分或随机窗口划分
3. 清洗缺失值
4. 用训练部分拟合标准化器，并对全部划分裁剪、标准化
5. 先构造未加权训练窗口并生成有标签掩码
6. 用有标签训练窗口计算相关性权重
7. 构造最终动态窗口x(t)、当前变量current_u(t)、延迟标签y(t+d)
8. 训练S-DDAE:
   8.1 全部训练窗口做重构预训练
   8.2 有标签窗口做监督微调
